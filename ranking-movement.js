(function(root,factory){
  ...
  function read(key){try{const raw=root.localStorage?.getItem(PREFIX+key);return raw?JSON.parse(raw):null}catch(_){return null}}
  function write(key,state){try{root.localStorage?.setItem(PREFIX+key,JSON.stringify(state))}catch(_){}return state}
  ...
  function ensureStyles(doc=root.document){if(!doc||doc.getElementById('bizimSkorRankingMovementStyle'))return false;const style=doc.createElement('style');style.id='bizimSkorRankingMovementStyle';style.textContent='.bs-rank-move{display:inline-flex;align-items:center;margin-left:4px;font-size:11px;font-weight:900;white-space:nowrap;vertical-align:middle}.bs-rank-up{color:#16a34a}.bs-rank-down{color:#dc2626}html[data-theme="dark"] .bs-rank-up{color:#4ade80}html[data-theme="dark"] .bs-rank-down{color:#f87171}';doc.head.appendChild(style);return true}
