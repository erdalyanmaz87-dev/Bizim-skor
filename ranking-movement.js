(function(root,factory){
  ...
  const PREFIX='bizimSkorRankingMovement:v4:';
  ...
  function nextState(previous,rows,revision){
    ...
    if(previous&&Object.keys(previousSnapshot).length)currentRows.forEach(row=>{const key=normalizeName(row.name),before=previousSnapshot[key];if(!before)return;if(Number(before.rank)>Number(row.rank))movement[key]={direction:'up'};else if(Number(before.rank)<Number(row.rank))movement[key]={direction:'down'}});
  ...
  function badge(movement){if(!movement?.direction)return'';const up=movement.direction==='up',arrow=up?'▲':'▼',cls=up?'bs-rank-up':'bs-rank-down';return ` <span class="bs-rank-move ${cls}" title="Önceki maç sonucuna göre ${up?'yükseldi':'geriledi'}" aria-label="${up?'Yükseldi':'Geriledi'}">${arrow}</span>`}
  function rankHtml(rank,movement,medal=''){return `${medal}${rank}${badge(movement)}`}
  function ensureStyles(doc=root.document){if(!doc||doc.getElementById('bizimSkorRankingMovementStyle'))return false;const style=doc.createElement('style');style.id='bizimSkorRankingMovementStyle';style.textContent='.bs-rank-cell{text-align:center!important;vertical-align:middle}.bs-rank-move{display:flex;width:20px;min-width:20px;height:20px;align-items:center;justify-content:center;margin:3px auto 0;padding:0;border-radius:999px;font-size:12px;font-weight:900;line-height:1;white-space:nowrap}.bs-rank-up{color:#16a34a!important;background:rgba(22,163,74,.12)}.bs-rank-down{color:#dc2626!important;background:rgba(220,38,38,.12)}html[data-theme="dark"] .bs-rank-up{color:#4ade80!important;background:rgba(74,222,128,.12)}html[data-theme="dark"] .bs-rank-down{color:#f87171!important;background:rgba(248,113,113,.12)}';doc.head.appendChild(style);return true}
