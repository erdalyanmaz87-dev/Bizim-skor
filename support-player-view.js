(function(root,factory){
  ...
  function supportButton(unread=0){return `<button type="button" id="openSupportInbox" class="support-contact-button">📩 Bize Ulaşın${unread>0?` <span class="support-badge">${unread}</span>`:''}</button>`}
  function form({anonymous=false}={}){return `<form id="supportRequestForm">${anonymous?'<label>Adın / oyuncu adın</label><input id="supportGuestName" maxlength="60" autocomplete="name" placeholder="Adını yaz">':''}<label>Kategori</label><select id="supportCategory"><option value="login_pin">Giriş / PIN</option><option value="predictions">Tahminler</option><option value="points_ranking">Puan / Sıralama</option><option value="technical">Teknik Sorun</option><option value="suggestion">Öneri</option><option value="other">Diğer</option></select><label>Mesajın</label><textarea id="supportMessage" maxlength="500" rows="5"></textarea><div class="small"><span id="supportCounter">0</span>/500</div><button type="submit" class="full p">Gönder</button></form>`}
  ...
  function panel(rows=[],options={}){return `<section class="support-panel"><div class="support-head"><h2>📩 Bize Ulaşın</h2><button type="button" data-support-close>×</button></div>${form(options)}<h3>Mesajlarım</h3><div id="supportHistory">${rows.map(card).join('')||'<p class="small">Henüz mesajın yok.</p>'}</div></section>`}
