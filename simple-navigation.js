(function(root,factory){
  ...
  function triggerAccount(doc,action){if(action==='inviteChampion'){openFeature(doc,'inviteChampion');return}const map={update:'bsHeaderUpdate',museum:'bsHeaderMuseumAction',logout:'bsHeaderAccountAction',login:'bsHeaderAccountAction',support:'bsHeaderSupport'};doc.getElementById(map[action])?.click()}
  ...
  function mount(doc=typeof document!=='undefined'?document:null){
    ...
    setTimeout(()=>{syncPredictionStatus(doc);moveInviteShell(doc)},1000);setTimeout(()=>{syncPredictionStatus(doc);moveInviteShell(doc)},2400);root?.addEventListener?.('bizimskor:session-ready',()=>setTimeout(()=>{renderDrawer(doc);moveInviteShell(doc);syncPredictionStatus(doc)},500));return true;
  }
  return Object.freeze({canonicalTab,preferredTabNames,bottomTargetForTab,predictionProgress,groupedItems,bottomMarkup,drawerMarkup,featureMarkup,styleMarkup,availableTabs,findOriginalTab,currentOriginalTab,mount,openDrawer,closeDrawer,openFeature,closeFeature});
});
