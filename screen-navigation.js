...
function cloneScreen(screen={}){const id=canonicalScreenId(screen.id||'home'),def=screenDefinition(id);return{id,title:String(screen.title||def?.title||''),scrollY:Number.isFinite(Number(screen.scrollY))?Math.max(0,Number(screen.scrollY)):0,context:screen.context??null}}
...
function pushScreen(state,screen){const base=Array.isArray(state?.stack)&&state.stack.length?state.stack.map(cloneScreen):[cloneScreen(ROOT_SCREEN)],next=cloneScreen(screen);if(next.id==='home')return{stack:[cloneScreen(ROOT_SCREEN)]};return{stack:[...base,next]}}
...
function rememberScroll(state,scrollY){const stack=Array.isArray(state?.stack)&&state.stack.length?state.stack.map(cloneScreen):[cloneScreen(ROOT_SCREEN)],index=stack.length-1;stack[index]={...stack[index],scrollY:Math.max(0,Number(scrollY)||0)};return{stack}}
...
if(typeof module==='object'&&module.exports)module.exports=api;else globalThis.BizimSkorScreenNavigation=api;
