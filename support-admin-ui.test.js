...
test('refresh mounts admin button only after server admin check',async()=>{
  const events={};const button={addEventListener:(name,fn)=>events[name]=fn};let mounted=0;
  const ui=createAdminSupportUI({doc:{getElementById:()=>null},view:{inboxButton:n=>`admin:${n}`},controller:{load:async()=>({rows:[{id:1},{id:2}],status:'new'})},adminApi:{isAdmin:async()=>true},placement:{insertAdminSupportButton:(_doc,node)=>{mounted++;assert.equal(node,button);return true}},makeElement:html=>{assert.equal(html,'admin:2');return button}});
  const result=await ui.refreshButton();assert.equal(result.admin,true);assert.equal(mounted,1);assert.equal(typeof events.click,'function');
});
