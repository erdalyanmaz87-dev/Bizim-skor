...
test('admin placement prefers stable profile action row',()=>{
  const actions={};const logout={parentElement:actions};const liveParent={};const live={parentElement:liveParent};
  const doc={getElementById:id=>id==='logoutPlayer'?logout:id==='adminLiveScorePanel'?live:null};
  const target=placement.findAdminSupportHost(doc);assert.equal(target.host,actions);assert.equal(target.before,logout);
});

test('admin placement falls back to live score panel parent',()=>{
...
test('admin placement returns null without anchors',()=>{const doc={getElementById:()=>null};assert.equal(placement.findAdminSupportHost(doc),null)});
