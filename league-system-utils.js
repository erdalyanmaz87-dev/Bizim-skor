(function(root,factory){
  ...
  function isLeagueEligible(validRoundCount){return Number(validRoundCount)>=2}
  function allocateLeagueCapacities(activeCount){
    ...
    const champions=Math.round(n*0.10),elite=Math.round(n*0.15),gold=Math.round(n*0.20),silver=Math.round(n*0.25);
    return{champions,elite,gold,silver,bronze:n-champions-elite-gold-silver};
  }
  function promotionSlots(capacities={}){
    const c={champions:Math.max(0,Number(capacities.champions)||0),elite:Math.max(0,Number(capacities.elite)||0),gold:Math.max(0,Number(capacities.gold)||0),silver:Math.max(0,Number(capacities.silver)||0),bronze:Math.max(0,Number(capacities.bronze)||0)};
    const slot=(value,ratio)=>value>0?Math.max(1,Math.round(value*ratio)):0;
    return{champions_elite:slot(c.champions,0.25),elite_gold:slot(c.elite,0.33),gold_silver:slot(c.gold,0.31),silver_bronze:slot(c.silver,0.31)};
  }
