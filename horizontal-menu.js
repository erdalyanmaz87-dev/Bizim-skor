(function(root,factory){
  ...
  const LABELS={live:'Hafta Sıralaması',weeklyRankings:'Hafta Sıralaması',sezu:'🏆 Sezu Ödül Sıralaması',general:'Genel Sıralama',resultsWeek:'Fikstür',footballCenter:'⚽ Futbol Merkezi',friendLeagues:'👥 Arkadaş Liglerim',history:'Tahmin Geçmişim',rules:'📜 Kurallar',chat:'💬 Sohbet'};
  const CANONICAL={weeklyRankings:'live'};
  function canonical(name){return CANONICAL[name]||name}
  function orderMenuTabs(names){const rows=names||[],actualByCanonical=new Map();rows.forEach(name=>{const key=canonical(name);if(!actualByCanonical.has(key)||name==='weeklyRankings')actualByCanonical.set(key,name)});return ORDER.map(key=>actualByCanonical.get(key)).filter(Boolean)}
  function menuLabel(name){return LABELS[name]||LABELS[canonical(name)]||name}
  ...
  return Object.freeze({canonical,orderMenuTabs,menuLabel,isPrimaryTab,mount});
});
