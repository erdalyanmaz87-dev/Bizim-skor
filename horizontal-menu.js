(function(root,factory){
  ...
  const ORDER=['live','sezu','general','fixture','footballCenter','friendLeagues','rules','chat'];
  const LABELS={live:'Hafta Sıralaması',sezu:'🏆 Sezu Ödül Sıralaması',general:'Genel Sıralama',fixture:'Fikstür',footballCenter:'⚽ Futbol Merkezi',friendLeagues:'👥 Arkadaş Liglerim',rules:'📜 Kurallar',chat:'💬 Sohbet'};
  ...
  function mount(){const tabs=document.querySelector('.tabs');if(!tabs||tabs.querySelector('.bs-scroll-menu'))return;ensureStyles();tabs.classList.add('bs-nav-ready');const wrap=document.createElement('div');wrap.className='bs-scroll-menu';const existing=[...tabs.querySelectorAll(':scope > .tab')];const byName=new Map(existing.map(tab=>[tab.dataset.tab,tab]));orderMenuTabs(existing.map(tab=>tab.dataset.tab)).forEach(name=>{const tab=byName.get(name);if(!tab)return;if(LABELS[name])tab.textContent=LABELS[name];wrap.appendChild(tab)});const pred=byName.get('pred');if(pred)pred.insertAdjacentElement('afterend',wrap);else tabs.appendChild(wrap)}
  return Object.freeze({orderMenuTabs,isPrimaryTab,mount});
});
