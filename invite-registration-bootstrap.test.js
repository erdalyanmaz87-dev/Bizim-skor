...
const {consumeRegistrationInvite}=require('./invite-registration-bootstrap');

test('records a game invite only after a new-player session exists',async()=>{
  const calls=[];
  const result=await consumeRegistrationInvite({
    invite:{inviterId:'Erdal',leagueId:null},
    token:'session-token',
    newPlayerName:'Ayse',
    api:{async rpc(name,args){calls.push([name,args]);return{data:true,error:null}}}
  });
...
test('passes the selected league id for league invite attribution',async()=>{
  let args;
  await consumeRegistrationInvite({
    invite:{inviterId:'Erdal',leagueId:'11111111-1111-1111-1111-111111111111'},
    token:'session-token',
    newPlayerName:'Ayse',
    api:{async rpc(name,value){args={name,value};return{data:true,error:null}}}
  });
  assert.deepEqual(args,{name:'record_player_invite',value:{p_token:'session-token',p_inviter_name:'Erdal',p_league_id:'11111111-1111-1111-1111-111111111111'}});
});

test('rejects self invite before rpc call',async()=>{
  let called=false;
  const result=await consumeRegistrationInvite({
    invite:{inviterId:'Ayse'},token:'session-token',newPlayerName:' ayse ',
    api:{async rpc(){called=true}}
  });
