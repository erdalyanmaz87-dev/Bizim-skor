(function(){
  window.addEventListener('DOMContentLoaded',()=>{
    ...
    function playerLabel(name){return window.BizimSkorPlayerIdentity?.playerLabel?.(name)||esc(name)}
    ...
    renderScoreTable=function(targetId,ps,rs){
      ...
      document.getElementById(targetId).innerHTML=rows.length
        ?`<table><tr><th>Sıra</th><th>Katılımcı</th><th>Puan</th><th>🎯</th><th>⚽</th></tr>${rows.map(r=>`<tr><td>${r.rank===1?'🥇 1':r.rank===2?'🥈 2':r.rank===3?'🥉 3':r.rank}</td><td>${playerLabel(r.name)}</td><td><b>${r.pts}</b></td><td>${r.ex}</td><td>${r.cr}</td></tr>`).join('')}</table>`
        :'Henüz puan oluşmadı.';
    };
    ...
    loadGeneral=async function(){
      ...
      document.getElementById('generalBoard').innerHTML=`<table><tr><th>Sıra</th><th>Katılımcı</th><th>Puan</th><th>🎯</th></tr>${rows.map(r=>`<tr><td>${r.rank===1?'🥇 1':r.rank===2?'🥈 2':r.rank===3?'🥉 3':r.rank}</td><td>${playerLabel(r.name)}</td><td><b>${r.pts}</b></td><td>${r.ex}</td></tr>`).join('')}</table>`;
    };
  });
})();
