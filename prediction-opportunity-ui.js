(function(root,factory){
 ...
 const apply=()=>{try{if(typeof selectedPredictionWeek==='undefined'||![4,5].includes(+selectedPredictionWeek))return;const box=document.getElementById('fx');if(!box)return;const rows=[...box.querySelectorAll('.m')],saved=[...box.querySelectorAll('.savedrow')],index=typeof fixtures!=='undefined'?api.findOpportunityIndex(fixtures,selectedPredictionWeek):-1,row=rows[index]||saved[index];if(!row)return;if(!row.classList.contains('prediction-opportunity-match'))styleRow(row);if(!(row.nextElementSibling?.classList.contains('prediction-opportunity-detail')))row.insertAdjacentHTML('afterend',detail())}catch(e){console.warn('prediction opportunity badge',e)}};
 const observer=new MutationObserver(apply);const mount=()=>{const fx=document.getElementById('fx');if(fx)observer.observe(fx,{childList:true,subtree:true});apply()};if(document.readyState==='complete')mount();else window.addEventListener('load',mount);document.addEventListener('change',e=>{if(e.target?.id==='predictionWeekSelect')setTimeout(apply,0)});
 ...
 function shouldDecorate(h,d){return!(h&&d)}return{isOpportunity,findOpportunityIndex,savedRowOpportunity,opportunityLayout,shouldDecorate};});
