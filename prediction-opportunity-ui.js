(function(root,factory){
 ...
 const apply=()=>{try{if(typeof selectedPredictionWeek==='undefined'||+selectedPredictionWeek!==5)return;const index=typeof fixtures!=='undefined'?api.findOpportunityIndex(fixtures,selectedPredictionWeek):-1;if(index<0)return;const fx=document.getElementById('fx'),state=document.getElementById('predictionState');const row=(fx&&findRow(fx,index,'.m'))||(state&&findRow(state,index,'.savedrow'));if(!row)return;if(!row.classList.contains('prediction-opportunity-match'))styleRow(row);addBadge(row);if(!(row.nextElementSibling?.classList.contains('prediction-opportunity-detail')))row.insertAdjacentHTML('afterend',detail())}catch(e){console.warn('prediction opportunity badge',e)}};
 ...
 function isOpportunity(f,w){return +w===5&&+f?.week===5&&+(f?.id??f?.fixture_id)===44}
 function findOpportunityIndex(fs,w){return(fs||[]).findIndex(f=>isOpportunity(f,w))}
 function savedRowOpportunity(r){return +r?.week===5&&+(r?.fixture_id??r?.id)===44}
