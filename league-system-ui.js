(function(root,factory){
  ...
  function renderLeagueSummary(summary={}){
    ...
    const perf=summary.performance_score==null?'—':Number(summary.performance_score).toFixed(2),status=summary.promotion_status||'none',participation=Math.min(2,Number(summary.valid_round_count)||0);
    ...
    return `<button type="button" class="league-summary league-summary-button${summary.is_eligible?'':' league-summary-pending'}" data-league-open="1"><span class="league-summary-title">${ICONS[code]||'🥉'} ${esc(label(code))}</span><strong class="league-summary-rank">${rank||'—'} / ${size||'—'}</strong><span class="league-summary-performance">Ort. Arena Puanı ${esc(perf)}</span><span class="league-summary-participation">Katılım ${participation} / 2</span><span class="league-summary-status">${esc(statusText)}</span><span class="league-summary-action">Arena'ya Gir ›</span></button>`;
  }
