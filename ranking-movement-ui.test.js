...
test('ayni rozet tekrar eklenmez',()=>{let inserted=0;const existing={textContent:'▲ 2',classList:{contains:n=>n==='bs-rank-up'},remove(){}};const node={classList:{add(){}},querySelector:()=>existing,insertAdjacentHTML:()=>inserted++};assert.equal(ui.reconcileBadge(node,{direction:'up',amount:2},{badge:()=>''}),false);assert.equal(inserted,0)});
test('hareket varsa sira hucresini ortalama sinifi ekler',()=>{const classes=[];const node={classList:{add:n=>classes.push(n)},querySelector:()=>null,insertAdjacentHTML(){}};ui.reconcileBadge(node,{direction:'down',amount:3},{badge:()=>'<span></span>'});assert.deepEqual(classes,['bs-rank-cell'])});
