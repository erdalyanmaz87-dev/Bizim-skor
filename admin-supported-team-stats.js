(function(root,factory){
  ...
  function render(data={}){
    ...
    const rows=(title,list,detail)=>`<section class="bs-admin-stats-group bs-admin-supported-team-group"><h3>${esc(title)} <small>(${list.length})</small></h3><div class="bs-admin-stats-list compact bs-admin-stats-columns">${list.length?list.map(x=>`<div><button type="button" class="bs-admin-player" data-admin-player="${esc(x.player_name)}"><b>${esc(x.player_name)}</b><span>${esc(detail(x))}</span></button></div>`).join(''):'<p>Bu grupta oyuncu yok.</p>'}</div></section>`;
    return `<section class="bs-admin-supported-team-summary" data-admin-supported-team-summary><div class="bs-admin-stats-grid"><b>Takım Seçen <strong>${Number(data.selected_count??selected.length)}</strong></b><b>Takım Seçmeyen <strong>${Number(data.unselected_count??unselected.length)}</strong></b></div>${rows('Takım Seçen Oyuncular',selected,x=>TEAM_NAMES[x.supported_team]||x.supported_team||'—')}${rows('Takım Seçmeyen Oyuncular',unselected,()=> 'Henüz seçim yapmadı')}</section>`;
  }
