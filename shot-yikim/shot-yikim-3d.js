...
const GROUND_Y=-.18;
const LAUNCH={x:0,y:1.5,z:2.7};
const BALL_RADIUS=.10;
const BALL_DENSITY=9;
const BRIDGE_DENSITY_FACTOR=.35;
const SHOT_COOLDOWN_MS=650;
const MATERIAL={wood:{color:0xb96a27,density:.65,friction:.52,restitution:.08},stone:{color:0x7b838f,density:1.25,friction:.58,restitution:.04},metal:{color:0x7b8798,density:1.75,friction:.44,restitution:.06}};
...
export async function createGame3D({container,level,onBlockFallen=()=>{},onSettled=()=>{}}){
  ...
  const tableBody=world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(0,TABLE.topY-TABLE.thickness/2,0));world.createCollider(RAPIER.ColliderDesc.cuboid(TABLE.width/2,TABLE.thickness/2,TABLE.depth/2).setFriction(.72).setRestitution(.02),tableBody);
  ...
  let entries=[],footballBody=null,footballMesh=null,shotTrail=null,disposed=false,enabled=true,fallenIds=new Set(),shotActive=false,quietFrames=0,lastShotTime=0,nextShotAt=0,raf=0;
  function clearShotTrail(){if(!shotTrail)return;scene.remove(shotTrail);shotTrail.geometry.dispose();shotTrail.material.dispose();shotTrail=null;}
  function clearTower(){clearShotTrail();for(const e of entries){scene.remove(e.mesh);world.removeRigidBody(e.body);}entries=[];fallenIds.clear();}
  function createBlock(block){const b=toWorldBlock(block);const mat=MATERIAL[b.material]||MATERIAL.stone;const isBridge=b.size.x>=.85&&b.size.y<=.32;const densityScale=isBridge?BRIDGE_DENSITY_FACTOR:1;let geom,colliderDesc;if(b.shape==='cylinder'){const radius=Math.min(b.size.x,b.size.z)*.48;geom=new THREE.CylinderGeometry(radius,radius,b.size.y,20);colliderDesc=RAPIER.ColliderDesc.cylinder(b.size.y/2,radius);}else{geom=boxGeometry(b.size);colliderDesc=RAPIER.ColliderDesc.cuboid(b.size.x/2,b.size.y/2,b.size.z/2);}const mesh=new THREE.Mesh(geom,makeMaterial(b.material));mesh.position.set(b.position.x,b.position.y,b.position.z);mesh.rotation.y=b.rotationY||0;mesh.castShadow=true;mesh.receiveShadow=true;mesh.userData.blockId=b.id;scene.add(mesh);const desc=RAPIER.RigidBodyDesc.dynamic().setTranslation(b.position.x,b.position.y,b.position.z).setRotation({x:0,y:Math.sin((b.rotationY||0)/2),z:0,w:Math.cos((b.rotationY||0)/2)}).setLinearDamping(.03).setAngularDamping(.02);const body=world.createRigidBody(desc);colliderDesc.setDensity(mat.density*densityScale).setFriction(isBridge?Math.min(mat.friction,.38):mat.friction).setRestitution(mat.restitution);world.createCollider(colliderDesc,body);entries.push({id:b.id,mesh,body,size:b.size});}
  function loadLevel(next){clearTower();for(const b of next.blocks)createBlock(b);resetFootball();quietFrames=0;shotActive=false;nextShotAt=0;}
  function resetFootball(){if(footballBody){world.removeRigidBody(footballBody);footballBody=null;}if(footballMesh){scene.remove(footballMesh);footballMesh=null;}footballMesh=new THREE.Mesh(new THREE.SphereGeometry(BALL_RADIUS,28,20),new THREE.MeshStandardMaterial({color:0xffffff,roughness:.52}));footballMesh.castShadow=true;footballMesh.position.set(LAUNCH.x,LAUNCH.y,LAUNCH.z);scene.add(footballMesh);const desc=RAPIER.RigidBodyDesc.dynamic().setTranslation(LAUNCH.x,LAUNCH.y,LAUNCH.z).setLinearDamping(.03).setAngularDamping(.03).setCcdEnabled(true);footballBody=world.createRigidBody(desc);world.createCollider(RAPIER.ColliderDesc.ball(BALL_RADIUS).setDensity(BALL_DENSITY).setFriction(.42).setRestitution(.20),footballBody);footballBody.setEnabled(false);}
  loadLevel(level);
  ...
  function shootAtClientPoint(clientX,clientY){const now=performance.now();if(!enabled||now<nextShotAt)return false;const target=clientPointToWorld(clientX,clientY);resetFootball();footballBody.setEnabled(true);footballBody.setTranslation(LAUNCH,true);footballBody.setLinvel({x:0,y:0,z:0},true);footballBody.setAngvel({x:0,y:0,z:0},true);const dir=target.clone().sub(new THREE.Vector3(LAUNCH.x,LAUNCH.y,LAUNCH.z)).normalize();const distance=Math.min(5.5,Math.max(2,target.distanceTo(new THREE.Vector3(LAUNCH.x,LAUNCH.y,LAUNCH.z))));const speed=10.7+distance*.48;footballBody.setLinvel({x:dir.x*speed,y:dir.y*speed,z:dir.z*speed},true);footballBody.setAngvel({x:-dir.y*7,y:dir.x*5,z:-dir.x*7},true);clearShotTrail();const trailGeometry=new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(LAUNCH.x,LAUNCH.y,LAUNCH.z),target.clone()]);const trailMaterial=new THREE.LineBasicMaterial({color:0xffffff,transparent:true,opacity:.52});shotTrail=new THREE.Line(trailGeometry,trailMaterial);scene.add(shotTrail);setTimeout(()=>{if(shotTrail)clearShotTrail();},260);shotActive=true;quietFrames=0;lastShotTime=now;nextShotAt=now+SHOT_COOLDOWN_MS;return true;}
  ...
  function loop(){if(disposed)return;world.step();for(const e of entries)syncEntry(e);if(footballBody){const p=footballBody.translation(),q=footballBody.rotation();footballMesh.position.set(p.x,p.y,p.z);footballMesh.quaternion.set(q.x,q.y,q.z,q.w);}if(shotActive){if(isSettled())quietFrames++;else quietFrames=0;if((quietFrames>22&&performance.now()>=nextShotAt)||performance.now()-lastShotTime>2600){shotActive=false;quietFrames=0;onSettled({remaining:getRemainingBlockCount()});if(performance.now()>=nextShotAt)resetFootball();}}renderer.render(scene,camera);raf=requestAnimationFrame(loop);}
  loop();
  function dispose(){disposed=true;cancelAnimationFrame(raf);clearShotTrail();ro.disconnect();renderer.dispose();container.innerHTML='';}
