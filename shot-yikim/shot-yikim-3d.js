...
const MATERIAL={wood:{color:0xb96a27,density:.65,friction:.58,restitution:.08},stone:{color:0x7b838f,density:1.45,friction:.66,restitution:.04},metal:{color:0x7b8798,density:2.2,friction:.48,restitution:.06}};
...
export async function createGame3D({container,level,onBlockFallen=()=>{},onSettled=()=>{}}){
  ...
  const tableBody=world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(0,TABLE.topY-TABLE.thickness/2,0));world.createCollider(RAPIER.ColliderDesc.cuboid(TABLE.width/2,TABLE.thickness/2,TABLE.depth/2).setFriction(.82).setRestitution(.02),tableBody);
  ...
  let entries=[],footballBody=null,footballMesh=null,shotTrail=null,disposed=false,enabled=true,fallenIds=new Set(),shotActive=false,quietFrames=0,lastShotTime=0,raf=0;
