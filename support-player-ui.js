...
function createPlayerSupportUI({doc,view,controller,placement,isUnread,makeElement}={}){
  ...
  async function render(rows){
    ...
    shell.innerHTML=`<div class="support-modal-backdrop" data-support-close></div><div class="support-modal-card">${view.panel(rows)}</div>`;
    ...
    form?.addEventListener('submit',async event=>{
      ...
      try{const result=await controller.send(category,text);if(!result.ok){showStatus(shell,result.error||'Mesaj gönderilemedi');return}await render(result.rows||[]);await refresh()}
  ...
  async function open(){
    ...
    if(unseen.length){await Promise.allSettled(unseen.map(row=>controller.markSeen(row.id)));await refresh()}
    return state;
  }
