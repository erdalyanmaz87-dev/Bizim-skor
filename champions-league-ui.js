(function(){
  ...
  function toolsMarkup(f,canApply=true){
    ...
    return `<div class="champions-tools">${canApply?`<button type="button" class="champions-tool" data-champions-robot="${f.id}" ${robot?'':'disabled'}>${label}</button>`:''}<button type="button" class="champions-tool" data-champions-stats="${f.id}">📊 Maç İstatistikleri</button></div>`;
  }
