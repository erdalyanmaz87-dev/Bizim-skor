...
declare
  ...
  v_updated text;
begin
  select pg_get_functiondef('public.get_friend_league_ranking(text,uuid)'::regprocedure)
    into v_definition;
  v_updated := replace(
    v_definition,
  ...
  select pg_get_functiondef('public.get_champions_league_ranking(text,text)'::regprocedure)
    into v_definition;
  v_updated := replace(
    v_definition,
  ...
  select pg_get_functiondef('public.get_champions_league_history(text,text,integer)'::regprocedure)
    into v_definition;
  v_updated := replace(
    v_definition,
    'rank() over(order by s.points desc,s.exact_count desc,s.correct_count desc)',
